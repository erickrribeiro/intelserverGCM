...
class DbHandler {
    ...
    public function getUserPaciente($user_id) {
        $stmt = $this->conn->prepare("SELECT * FROM users where user_id not in (Select idPaciente from paciente_familiar where idFamiliar = ? ) ");
        $stmt->bind_param("s", $user_id);
        $stmt->execute();
    ...
    public function getParents($user_id) {
        ...
        $query = "select * from users where user_id in (select idFamiliar from paciente_familiar  where idPaciente = ?)";
    ...
    // messaging in a chat room / to persional message
    public function addParente($id_paciente, $id_familiar) {
        $response = array();

        $stmt = $this->conn->prepare("INSERT INTO paciente_familiar VALUES(?, ?)");
        $stmt->bind_param("ii", $id_paciente,$id_familiar);
        ...
        if ($result = $stmt->execute()) {
            ...
            $response['error'] = true;
            $response['message'] = 'Não foi possivel salvar' . $stmt->error;
        }
